...
-- Globals to be updated in case of an title update (bruh)
...
local function add_info_option(submenu, text, funcget, forceplayer, cb)
...
local function PopulateSubmenus(subtp, subtroll, subgiveveh, subinfo, enterfunc)
 
	-- Teleport Options
	add_info_option(subtp, "Player", function() return player.get_player_name(selectedplayer) end, nil, enterfunc)
	...
	--Trolling Options
	add_info_option(subtroll, "Player", function() return player.get_player_name(selectedplayer) end, nil, enterfunc)
	...
	-- Info Panel
	add_info_option(subinfo, "Player", function() return player.get_player_name(selectedplayer) end, nil, enterfunc)
...
local function BuildOP()
	local subtp = menu.add_player_submenu("Teleport Options")
	local subtroll = menu.add_player_submenu("Trolling Options")
	local subgiveveh = menu.add_player_submenu("Give Vehicle")
	local subinfo = menu.add_player_submenu("Player Info") --, function() selectedplayer = menu.get_selected_player_index() end)
 
	PopulateSubmenus(subtp, subtroll, subgiveveh, subinfo, function() selectedplayer = menu.get_selected_player_index() end)
end
...
local function BuildListSub()
	...
	for i = 0, 31 do
		...
		if ply then 
			...
			PopulateSubmenus(subtp, subtroll, subgiveveh, subinfo)
		end
	end
end
...
BuildOP()
playerlist = menu.add_submenu("AppleVegas's Player List", Update)